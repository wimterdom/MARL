...
/*RL_Fuzz*/
int selected_para[30];      // the times of every para selected 
int reward_para[30];        // the reward of every para
double effective_para[30];     // the effective of every para
double probability_para[30];   // the probability of every para
//u32 reward_exec_num[30];    // the reward of exec para number
double probability_num_argv[30]; // the probability of reward argv num exec
double effect_exec[30];        // the effective of exec para num
static u8 init_argv_flag = 1; // the flag of the init state argv generate
int cur_index;          // record the index of the current selected para 
int exec_para_count[30];   // record the executation para number of command line 
int argv_exec_count = 0;
int count_para = 0;
double avg_hit = 0;
int count_rl_time = 0;
int sum_hit = 0;
int last_new_hit = 0;
int final_hit = 0;
int select_index_count = 0;
int select_para_index[30];
int reset_count = 0;
...
static void bind_to_free_cpu(void)
{
  ...
  if (i == cpu_core_count)
  {
    ...
    SAYF("\n" cLRD "[-] " cRST
         ...
         "    other instances of RL-Fuzz (or similar CPU-locked tasks). Starting\n"
...
static void show_stats(void)
{
  ...
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "RL-Fuzz", use_banner);
...
static void rl_update()
{
    double sum_effective = 0;
    for(int i=0; i < parameter_total; i++)
    {
        if(selected_para[i] != 0)
            effective_para[i] = reward_para[i] / selected_para[i];
        else if(selected_para[i] == 0)
            effective_para[i] = 0;
    }

    for(int i =0; i < parameter_total; i++)
    {
        sum_effective += effective_para[i];
    }

    for(int i=0; i < parameter_total; i++)
    {
        if(sum_effective != 0 && sum_effective > 0)
            probability_para[i] = effective_para[i] / sum_effective;
        else if(sum_effective == 0 && sum_effective < 0)
            probability_para[i] = 0;
    }
}
...
/*init mutation use the regular , the second time use rl*/
/*record the times of exec every para & reward*/
/*compute the effective of every para & probability*/
/*use probability distributation to combine the para*/
...
static void random_generate_arg(char **new_argv, char **argv, int argv_array[])
{
  ...
  argv_index++;
  if(count_rl_time > 0)
    avg_hit = sum_hit / count_rl_time;
  else if(count_rl_time <= 0)
    avg_hit = 0;
  // add qemu argv
  if (qemu_mode)
  {
...
/*
  stage_short = "arg1";
  stage_name = "init_arg_gen";
  select_index_count = 0;
  memset(select_para_index, 0, sizeof(select_para_index));
  for (int i = 0; i < parameter_count; i++)
  {
    if (parameter[i].must) // must = true → must choose
    {
      int ur = UR(parameter[i].count); // generate the random num between 0 ~ num_subpara-1
      argv_array[i] = ur + 1; // store parameter + 1
      char *substr = NULL;
      char buf[parameter_strings_long];
      strcpy(buf, parameter[i].parameter[ur]); // choose parameter
      cur_index = para_index_table[i][ur];
      select_para_index[select_index_count] = cur_index;
      selected_para[cur_index]++; // record the para
      substr = strtok(buf, " ");
      count_para++;
      select_index_count++;
    ...
    else if (UR(2) != 0) // must = false → maybe not choose
    {
...
*/
  ...
  if(init_argv_flag || last_new_hit < avg_hit)  // the init time generate   last_new_hit < avg_hit)
  {
    stage_short = "arg1";
    stage_name = "init_arg_gen";
    select_index_count = 0;
  ...
  else//Exploit //!init_argv_flag && last_new_hit >= avg_hit
  {
    stage_short = "arg_RL";
    stage_name = "RL_arg_gen";
    select_index_count = 0;
    memset(select_para_index, 0, sizeof(select_para_index));
    for (int i = 0; i < parameter_count; i++)
    {
      ...
      else
      {
        ...
        {
          int random_argv_probability = UR(100);
          if((probability_para[para_index_table[i][j]] * 100) > random_argv_probability)
          {
            argv_array[i] = j + 1;
            char *substr = NULL;
            char buf[parameter_strings_long];
            strcpy(buf, parameter[i].parameter[j]);
            cur_index = para_index_table[i][j];
            select_para_index[select_index_count] = cur_index;
            selected_para[cur_index]++;  // record the para
            substr = strtok(buf, " ");
            count_para++;
...
/* argv fuzz_one */
/*reward: argv_fuzz_one → argv_common_fuzz_staff → save_if_intersting*/
...
static char **argv_fuzz_one(char **argv)
{
  ...
  {
    orig_hit_cnt = queued_paths + unique_crashes;
    new_argv = (char **)ck_alloc(sizeof(char *) * 600);
    memset(new_argv, 0, sizeof(char *) * 600);
    ...
    new_argv = NULL;
    new_hit_cnt = queued_paths + unique_crashes;
    final_hit = new_hit_cnt - last_new_hit;
    count_rl_time++;
    sum_hit += new_hit_cnt;
    last_new_hit = new_hit_cnt;
    if(new_hit_cnt - orig_hit_cnt > 0)
    {
      for(int i=0; i<select_index_count; i++)
      {
        reward_para[select_para_index[i]]++;
      }
      //reward_exec_num[count_para]++;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  SAYF(cCYA "RL-Fuzz " cBRI VERSION cRST " by <anroldwzachary@gmail.com>\n");
  ...
  /* the array of record para zeroed*/
  memset(selected_para, 0, sizeof(selected_para));
  memset(reward_para, 0, sizeof(reward_para));
  memset(effective_para, 0, sizeof(effective_para));
  memset(probability_para, 0, sizeof(probability_para));
  memset(exec_para_count, 0, sizeof(exec_para_count));
  //memset(reward_exec_num, 0, sizeof(reward_exec_num));
  memset(probability_num_argv, 0, sizeof(probability_num_argv));
  memset(effect_exec, 0, sizeof(effect_exec));
  ...
  while ((opt = getopt(argc, argv, "+i:o:s:wrf:m:b:t:T:dnCB:S:M:x:p:QV")) > 0)
    ...
    { /* master sync ID */
      ...
      FATAL("RL-Fuzz Can only be used in slave mode, -M options not supported. please try to use -S.");
      exit(0);
      // u8 *c;
  ...
  if (argv_fuzz_flag == 1) //第一次執行的多參數是否random
  {
