...
if(len(sys.argv) < 2):
    print("[Usage] python3 plot_crash.py [src1_plotdata_path] [label_1] [src2_plotdata_path] [label_2] [src3_plotdata_path] [label_3] [output_path] [output_name] [title]")
else:
    ...
    input_4 = str(sys.argv[7])
    label_4 = str(sys.argv[8])
    output_name = '/' + str(sys.argv[10]) + '.png'
    out_path = str(sys.argv[9]) + output_name
    title_name = str(sys.argv[11])
    ...
    df4 = pd.read_csv(input_4, header=None, index_col=None)
    #print(df.shape[0])
    ...
    for i in range(1, df4.shape[0]):
        final_time_4.append((int(df4[0][i]) - int(df4[0][1])) / 3600)
    ...
    # y4 data
    for  i in range(1, df4.shape[0]):
        unique_crash_4.append(int(df4[7][i]))

     # generate picture
    plt.title(title_name)
    plt.xlabel('Time(Hour)')
    plt.ylabel('crashes')
    plt.plot(final_time_1, unique_crash_1, color=(255/255, 0/255, 0/255), label=label_1)
    plt.plot(final_time_2, unique_crash_2, color=(0/255, 255/255, 0/255), label=label_2)
    plt.plot(final_time_3, unique_crash_3, color=(0/255, 0/255, 255/255), label=label_3)
    plt.plot(final_time_4, unique_crash_4, color=(255/255, 255/255, 0/255), label=label_4)
    plt.legend()
