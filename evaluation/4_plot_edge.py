...
if(len(sys.argv) < 2):
    ...
    out_dir = str(sys.argv[9]) + output_name
...
'''
    ...
    #y4 data
    for i in range(1, df4.shape[0]):
        map_size_percent_age4 = df4[6][i].strip().replace("%", "")
        mapsize4 = float(map_size_percent_age4)  * 65536 / 100
        edge_4.append(mapsize4)
    ...
    plt.plot(final_time_4, edge_4, color=(255/255, 255/255, 0/255), label=label_4)
    plt.legend()
    plt.savefig(out_dir)
    plt.show()
