...
# y2 data
for i in range(1, df2.shape[0]):
    map_size_percent_age = df2[6][i].strip().replace("%", "")
    mapsize = float(map_size_percent_age) * 65536 / 100
    unique_edge_2.append(mapsize)

# z2
for i in range(1, df3.shape[0]):
    map_size_percent_age = df3[6][i].strip().replace("%", "")
    mapsize = float(map_size_percent_age) * 65536 / 100
    unique_edge_3.append(mapsize)

 # generate picture
plt.title('Tiff2pdf')
plt.xlabel('Time(Minutes)')
plt.ylabel('Edge coverage')
plt.plot(df_time, unique_edge_1, color=(255/255, 100/255, 100/255), label='RL-adaptive')
plt.plot(df2_time, unique_edge_2, '--', color=(100/255, 100/255, 255/255), label='Yuan')
plt.plot(df3_time, unique_edge_3, color=(255/255, 0/255, 255/255), label='RL-reward')
plt.legend()
plt.savefig('../out/tiff2pdf_1h/compare_edge.png')
plt.show()
