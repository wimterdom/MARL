...
/*add out_dir path*/
void parameter_index_csv(char j[])
{
        FILE *fp;
        fp = fopen("./parameter_index.csv","a+");
        fprintf(fp, "%s\n", j);
        fclose(fp);
}

void index_table_csv(int j[])
{
    FILE *fpp;
    fpp = fopen("./index_table.csv", "a+");
    fprintf(fpp, "%d\n", j);
    fclose(fpp);
}
    ...
    while (cur_node != NULL)
    {
        ...
        if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"MUST")))
        {
            ...
            if (strcmp("true", must) == 0)
            {
                ...
                must_index_table[parameter_total] = 1;
            }
            else if (strcmp("false", must) == 0)
            {
                parameter[parameter_count].must = 0;
                must_index_table[parameter_total] = 0;
            }
        }
        else if ((!xmlStrcmp(cur_node->name, (const xmlChar *)"ELEMENT")))
        {
            ...
            if (strlen(element) < parameter_strings_long)
            {
                ...
                strncpy(parameter_index[parameter_total], element, strlen(element));
                para_index_table[parameter_count][p] = parameter_total;
                parameter_index_csv(parameter_index[parameter_total]);
                index_table_csv(para_index_table[parameter_count][p]);
            }
            else
            {
                exit(1);
